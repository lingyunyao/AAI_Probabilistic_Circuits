...
//    val notNaN_isInfOut = io.a.isInf || io.b.isInf
    ...
    val sDiffExps_reg = RegNext(sDiffExps, init = 0.U)
    ...
    val pre_shifted_sig_reg = RegNext(pre_shifted_sig, init = 0.U)

    val shifted_sig = Mux(modNatAlignDist > (sigWidth + 1).U, 0.U, pre_shifted_sig_reg)((sigWidth + 1)*2 - 1, 0)
    val shifted_sig_reg = RegNext(shifted_sig, init = 0.U)
    ...
    val sig_sum_reg = RegNext(sig_sum, init = 0.U)
    ...
    val fullrawout_exp_reg = RegNext(fullrawout_exp, init = 0.U)
    val fullrawout_sig_reg = RegNext(fullrawout_sig, init = 0.U)
